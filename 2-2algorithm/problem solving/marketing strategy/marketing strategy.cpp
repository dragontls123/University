...
double get_result(int left, int right) {
	...
	for (i = 0; i < num; i++) {
		...
		while (point >= 0 && (area[point].y >= area[i].y - min_result)) {
			double diff = sqrt((area[point].x - area[i].x) * (area[point].x - area[i].x) + (area[point].y - area[i].y) * (area[point].y - area[i].y));
			if (diff < min_result) {
				min_result = diff;
			}
			point--;
...
int main() {
	int i; int n;  double result;
	FILE* fp = fopen("input.txt", "r");
	fscanf(fp, "%d", &n);
	for (i = 0; i < n; i++) {
		fscanf(fp, "%lf %lf", &pt[i].x, &pt[i].y);
	}
